...
namespace Busycator.Layers
{
    public class Surface : WorldLayer
    {
        ...
        public SuperCity City { get; private set; }

        public override IEnumerable<ETerrains> DefaultEmptySpaces { get { yield return ETerrains.GRASS; } }

        public override IEnumerable<ETerrains> DefaultWalls { get { yield return ETerrains.RED_BRICK_WALL; } }
        ...
        private static void LoadNicks()
        {
            ...
            {
                if (BusycatorGame.XResourceRoot.NickInfos.Count > 0)
                {
                    var separator = new[] { ',' };
                    foreach (var nicksInfo in BusycatorGame.XResourceRoot.NickInfos)
                    {
