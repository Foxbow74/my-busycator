...
namespace GameCore.Mapping.Layers
{
	class TestSurfaceGenerator1x1
	{
		...
		{
			_block.AddObject(ThingHelper.AllThings().First(_furniture => _furniture.Is<ClosedDoor>()), new Point(2, 1));
			_block.AddObject(ThingHelper.AllThings().First(_furniture => _furniture.Is<ClosedDoor>()), new Point(1, 2));

			_block.AddObject(ThingHelper.AllItems().First(_item => _item.Is<Axe>()), new Point(4, 1));
			_block.AddObject(ThingHelper.AllItems().First(_item => _item.Is<Axe>()), new Point(3, 2));
		}
	}
}
